  ...
  <div class="grid_card">
    <GridCard :gridCardType="false" :status="document && document.status ? document.status : {}">
      <template v-slot:grid-card-header_title>
        ...
        <div v-else>
          <span class="grid_card_span" >Приемка импорт: нет данных</span>
        </div>
      </template>
              ...
              <Card  :titleSize="'16px'" :style="'margin-top: 20px'">
                ...
                <template v-slot:body-content>
                  <TableHistory
                    ...
                    :rows="!loading_history ? history.rows : []"
                  >
                  </TableHistory>
                </template>
              </Card>
...
export default {
  ...
  computed: {
    ...mapGetters({ document: 'documents/incoming/document' }),
    ...mapGetters({ tableIncomingHeaders: 'headers/tableIncomingHeaders' }),
  ...
  methods: {
    async getDocHistory() {
      ...
      await this.$store.dispatch('documents/documentHistory', { id: parseInt(this.$route.params.id), type: 'incoming_docs' })
    ...
    this.$watch(
      ...
      async () => {
        if (this.$route.params.id){
          ...
          this.$store.dispatch('documents/incoming/clearDocument')
          await this.$store.dispatch('documents/incoming/incomingDocumentById', parseInt(this.$route.params.id))
        }
      },
      { immediate: true }
    )
...
<style src="./IncomingCard.scss" lang="scss" scoped>
