...
export default {
  ...
  computed:{
    ...
    documents() {
      return this.$store.getters["documents/fundInformation/activeRecords/documents"]
    },
    pagination() {
      return this.$store.getters['pagination/pagination']('fundInformation')
    },
    filters() {
      return this.$store.getters['filters/filters']('activeRecords')
    },
    statuses() {
      return this.$store.getters['statuses/statuses']('active')
    }
  },
  ...
  beforeMount() {
    this.$store.dispatch('organizations/objects')
    this.$store.dispatch('statuses/getStatusesFund', 'active')
  },

  methods:{
    cleanFilters(){
      this.$store.dispatch('filters/cleanFilters', 'activeRecords')
    },
    saveFilters(){
      this.filters.doctype = { value: 'activeRecords', visibility: false, name: 'Тип документа' }
      this.$store.dispatch('filters/saveFilters', this.filters)
    },
    async applyFilters(){
      this.loading = true
      this.saveFilters()
      await this.$store.dispatch('documents/fundInformation/activeRecords/activeRecordDocuments',
        {
