  ...
  <div class="grid_card">
    <gridLayout :filters="true">
      ...
      <template v-slot:grid>
        <TableDocumentsList
          :headers="commissioningHeaders"
          :loading="loading"
          :rows="documents.rows"
          :typeDoc="'Commissioning'"
        >
        </TableDocumentsList>
      </template>
          ...
          <template v-slot:row>
            ...
            <div class="filters_column" v-show="filters.productGroup.visibility">
              <dropDownMenuInput :label="filters.productGroup.name" id="productGroup" v-model="filters.productGroup.value" :items="productGroup" >
                <template v-slot:input-header>
                  Товарная группа
                </template>
              </dropDownMenuInput>
            </div>
            <div class="filters_column" v-show="filters.introduceType.visibility">
              <dropDownMenuInput :label="filters.introduceType.name" id="introduceType" v-model="filters.introduceType.value" :items="introduceType">
              </dropDownMenuInput>
            </div>
...
export default {
  ...
  computed: {
    ...
    ...mapGetters({ commissioningHeaders: 'headers/commissioningHeaders' }),
    documents() {
      return this.$store.getters["documents/commissioning/documents"]
    },
    pagination() {
      return this.$store.getters['pagination/pagination']('commissioning')
    },
    filters() {
      return this.$store.getters['filters/filters']('Commissioning')
    },
    dateFrom() {
      let date = null
    ...
    statuses() {
      return this.$store.getters['statuses/statuses']('mark_introduce_docs')
    }
  },
  ...
  mounted() {
    ...
    this.$store.dispatch('statuses/getStatuses', 'mark_introduce_docs')
  },
  ...
  methods: {
    cleanFilters(){
      this.$store.dispatch('filters/cleanFilters', 'Commissioning')
    },
    saveFilters(){
      this.filters.doctype = { value: 'Commissioning', visibility: false, name: 'Тип документа' }
      this.$store.dispatch('filters/saveFilters', this.filters)
    },
    async applyFilters() {
      this.loading = true
      this.saveFilters()
      await this.$store.dispatch('documents/commissioning/commissioningDocuments', {
        transactionId: this.filters.transactionId.value ?? null,
        productGroupId: this.filters.productGroup.value?.id ?? null,
        introduceTypeId: this.filters.introduceType.value?.id ?? null,
...
<style lang="scss" src="./Commissioning.scss" scoped>
</style>
